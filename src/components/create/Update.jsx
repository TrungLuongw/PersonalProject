...
const Update = () => {
    ...
    useEffect(() => {
        async function fetchData() {
            try {
                const res1 = await categoryService.get();
                if (res1.isSuccess) {
                    setCategories(res1.data);
                }
                const res2 = await postService.getPost(id);
                if (res2.isSuccess) {
                    setPost(res2.data);
                    setAvatar(API_IMAGE + res2.data.avatar);
                }
            } catch (error) {
                console.log(error);
            }
    ...
    const handleUpdatePost = async (e) => {
        try {
            const response = await postService.putPost(post._id, post);
            if (response.isSuccess) {
        ...
        <Grid container spacing={8} className="create-container">
            ...
            <Grid item lg={10} sm={10} xs={10} style={{ position: 'relative' }}>
                ...
                <div className="create-group-button">
                    ...
                    <button className="create-public" onClick={(e) => handleUpdatePost(e)}>
                        Edit
                    </button>
                </div>
